...
import type { Jobs } from "viktor/viktor-database-client";
import CmsTable from "../../_components/cms-table";

export default function AbteilungenTable() {
  const router = useRouter();

  const Abteilungen = api.cms.getJobs.useQuery();
  const Löscher = api.cms.deleteJob.useMutation({
    onSuccess: () => {
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem asChild>
                <Link href={"/CMS/Jobs/" + x.id}>Bearbeiten</Link>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem
                onClick={async () => {
  ...
  return (
    <div className="mt-5">
      <Button asChild className="mb-5">
        <Link href="/CMS/Jobs/Neu">Neuen Job anlegen</Link>
      </Button>
